...
class SocketServer(Thread):
    def __init__(self, host = '0.0.0.0', port = credenziali2.port, max_clients = 5):
                ...
                if len(rdy_read) > 0:
                    ...
                    if len(read_data) == 0:
                        ...
                        try:
                            ...
                            gnss_data=pynmea2.parse(dati[0].decode('utf-8'))
                            print(gnss_data)
                            print('ora UTC {}'.format(ora))
                            #print('latitudine {}'.format(gnss_data.latitude))
                            #print('longitudine {}'.format(gnss_data.longitude))
                            #print('altitude {}'.format(gnss_data.altitude))
                            #print('qualita fissaggio {}'.format(gnss_data.gps_qual))
                            ...
                            quota=float(gnss_data.altitude)
                            print(lat)
                            print(lon)
                            print(quota)
                            #quality=int(dati[5])
                            ...
                            query='INSERT INTO demo_rfi.posizioni(thread, ip, data, lat, lon, quota) VALUES ({}, \'{}\', \'{}\', {}, {}, {});'.format(self.number, ip_addr[0], ora, lat, lon, quota);
                            #query='INSERT INTO demo_rfi.posizioni(thread, ip, data, lat, lon, quota VALUES ({}, \'{}\', \'{}\', {}, {}, {});'.format(self.number, ip_addr[0], gnss_data_ora, lat, lon, quota);
                            print(query)
                            curr.execute(query)
                            print('{} - Data saved on PostgreSQL'.format(ip_addr[0]))
                        except:
                            print('*******************************')
                            print('messaggio diverso dal GPGGA')
                            print('*******************************')
